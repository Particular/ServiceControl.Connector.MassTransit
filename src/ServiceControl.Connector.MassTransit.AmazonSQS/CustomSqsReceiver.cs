...
class CustomSqsReceiver(IMessageReceiver receiver) : IMessageReceiver
{
    ...
    public Task StartReceive(CancellationToken cancellationToken = new()) => receiver.StartReceive(cancellationToken);

    public Task ChangeConcurrency(PushRuntimeSettings limitations, CancellationToken cancellationToken = new()) => receiver.ChangeConcurrency(limitations, cancellationToken);

    public Task StopReceive(CancellationToken cancellationToken = new()) => receiver.StopReceive(cancellationToken);
